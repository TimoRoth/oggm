...
# Locals
from oggm import OGGM, tasks, utils
...
if __name__ == "__main__":
    ...
    oggm = OGGM()
    oggm.initialize(logging_level='WORKFLOW')
    rgi_version = '61'
    ...
    oggm.PATHS['working_dir'] = WORKING_DIR

    # RGI file
    path = oggm.utils.get_rgi_region_file(rgi_region, version=rgi_version)
    rgidf = dask.delayed(gpd.read_file)(path)

    @dask.delayed
    def filter_rgidf(rgidf):
    ...
    for f in factors:
        ...
        output_gdirs = oggm.execute_entity_task([
                tasks.mass_conservation_inversion(glen_a=glen_a*f, fs=0),
                tasks.filter_inversion_output
            ], gdirs)
        # Store the results of the inversion only
        res = utils.compile_glacier_statistics(output_gdirs, filesuffix=suf, inversion_only=True)

        # With sliding
        suf = '_{:03d}_with_fs'.format(int(f * 10))
        output_gdirs = oggm.execute_entity_task([
                tasks.mass_conservation_inversion(glen_a=glen_a*f, fs=fs),
                tasks.filter_inversion_output
            ], gdirs)
        # Store the results of the inversion only
        res = utils.compile_glacier_statistics(output_gdirs, filesuffix=suf, inversion_only=True)
