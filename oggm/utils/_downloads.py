...
class Downloads:
    def __init__(self, oggm, **kwargs):
        ...
        # all sample data
        self.SAMPLE_DATA_GH_REPO = 'OGGM/oggm-sample-data'
        self.SAMPLE_DATA_COMMIT = '40d1e12bce713ab1155d83e5f77b9e070151be41'

        self.GDIR_URL = 'https://cluster.klima.uni-bremen.de/~oggm/gdirs/oggm_v1.1/'
        self.DEMO_GDIR_URL = 'https://cluster.klima.uni-bremen.de/~oggm/demo_gdirs/'
        self.DEMS_GDIR_URL = 'https://cluster.klima.uni-bremen.de/data/gdirs/dems_v0/'

        self.CMIP5_URL = 'https://cluster.klima.uni-bremen.de/~nicolas/cmip5-ng/'

        self.CHECKSUM_URL = 'https://cluster.klima.uni-bremen.de/data/downloads.sha256.hdf'
        self.CHECKSUM_VALIDATION_URL = self.CHECKSUM_URL + '.sha256'

        # Web mercator proj constants
        self.WEB_N_PIX = 256
        self.WEB_EARTH_RADUIS = 6378137.

        self.DEM_SOURCES = ['GIMP', 'ARCTICDEM', 'RAMP', 'TANDEM', 'AW3D30', 'MAPZEN',
    ...
    def get_lock(self):
        ...
        if self.lock is None:
            # Global Lock
            if self.oggm.PARAMS.get('use_mp_spawn', False):
    ...
    def get_dl_verify_data(self, section):
        ...
        if self.oggm.DATA.get(verify_key) is not None:
            return self.oggm.DATA[verify_key]

        verify_file_path = os.path.join(self.oggm.CACHE_DIR, 'downloads.sha256.hdf')

        def verify_file():
            """Check the hash file's own hash"""
            logger.info('Checking the download verification file checksum...')
            try:
                with requests.get(self.CHECKSUM_VALIDATION_URL) as req:
        ...
        if not np.any(['dl_verify_data_' in k for k in self.oggm.DATA.keys()]):
        ...
        if not os.path.isfile(verify_file_path):
            logger.info('Downloading %s to %s...'
                        % (self.CHECKSUM_URL, verify_file_path))

            with requests.get(self.CHECKSUM_URL, stream=True) as req:
        ...
        self.oggm.DATA[verify_key] = data

        return data


    def _call_dl_func(self, dl_func, cache_path):
        """Helper so the actual call to downloads can be overridden
        """
        return dl_func(cache_path)


    def _cached_download_helper(self, cache_obj_name, dl_func, reset=False):
        ...
        """
        cache_dir = self.oggm.PATHS['dl_cache_dir']
        cache_ro = self.oggm.PARAMS['dl_cache_readonly']
        try:
            # this is for real runs
            fb_cache_dir = os.path.join(self.oggm.PATHS['working_dir'], 'cache')
            check_fb_dir = False
        except KeyError:
            # Nothing have been set up yet, this is bad - use tmp
            # This should happen on RO cluster only but still
            fb_cache_dir = os.path.join(self.oggm.CACHE_DIR, 'cache')
            check_fb_dir = True
        ...
        if not self.oggm.PARAMS['has_internet']:
        ...
        try:
            cache_path = self._call_dl_func(dl_func, cache_path)
    ...
    def _verified_download_helper(self, cache_obj_name, dl_func, reset=False):
        """Helper function for downloads.
        ...
        """
        path = self._cached_download_helper(cache_obj_name, dl_func, reset)

        try:
            dl_verify = self.oggm.PARAMS['dl_verify']
        except KeyError:
            dl_verify = True

        if dl_verify and path and cache_obj_name not in self.oggm.DL_VERIFIED:
            cache_section, cache_path = cache_obj_name.split('/', 1)
            data = self.get_dl_verify_data(cache_section)
            if cache_path not in data.index:
                logger.info('No known hash for %s' % cache_obj_name)
                self.oggm.DL_VERIFIED[cache_obj_name] = True
            else:
                # compute the hash
                sha256 = hashlib.sha256()
    ...
    def _requests_urlretrieve(self, url, path, reporthook, auth=None, timeout=None):
    ...
    def _classic_urlretrieve(self, url, path, reporthook, auth=None, timeout=None):
    ...
    def _ftps_retrieve(self, url, path, reporthook, auth=None, timeout=None):
    ...
    def _get_url_cache_name(self, url):
    ...
    def oggm_urlretrieve(self, url, cache_obj_name=None, reset=False,
        ...
        if cache_obj_name is None:
            cache_obj_name = self._get_url_cache_name(url)

        def _dlf(cache_path):
            logger.info("Downloading %s to %s..." % (url, cache_path))
            try:
                self._requests_urlretrieve(url, cache_path, reporthook, auth, timeout)
            except requests.exceptions.InvalidSchema:
                if 'ftps://' in url:
                    self._ftps_retrieve(url, cache_path, reporthook, auth, timeout)
                else:
                    self._classic_urlretrieve(url, cache_path, reporthook, auth,
                                              timeout)
            return cache_path

        return self._verified_download_helper(cache_obj_name, _dlf, reset)


    def _progress_urlretrieve(self, url, cache_name=None, reset=False,
        ...
        try:
            ...
            def _upd(count, size, total):
                ...
                sys.stdout.flush()
            res = self.oggm_urlretrieve(url, cache_obj_name=cache_name, reset=reset,
            ...
            return self.oggm_urlretrieve(url, cache_obj_name=cache_name,
                                         reset=reset, auth=auth, timeout=timeout)


    def aws_file_download(self, aws_path, cache_name=None, reset=False):
        with self.get_lock():
            return self._aws_file_download_unlocked(aws_path, cache_name, reset)


    def _aws_file_download_unlocked(self, aws_path, cache_name=None, reset=False):
    ...
    def file_downloader(self, www_path, retry_max=5, cache_name=None,
        ...
        while retry_counter <= retry_max:
            ...
            try:
                retry_counter += 1
                local_path = self._progress_urlretrieve(www_path, cache_name=cache_name,
                ...
                if (self.oggm.PATHS['dl_cache_dir'] and
                      err.path.startswith(self.oggm.PATHS['dl_cache_dir']) and
                      self.oggm.PARAMS['dl_cache_readonly']):
                    if not cache_name:
                        cache_name = self._get_url_cache_name(www_path)
    ...
    def locked_func(self, func):
        """To decorate a function that needs to be locked for multiprocessing"""
        # TODO: Replace with file-based lock in Cache-Dir
        @wraps(func)
    ...
    def file_extractor(self, file_path):
        ...
        # extract directory
        tmpdir = self.oggm.PATHS['tmp_dir']
        mkdir(tmpdir)
        ...
        self.oggm.get_lru_handler(tmpdir, ending=of_ext).append(o_path)

        return o_path


    def download_with_authentication(self, wwwfile, key):
        ...
        cache_obj_name = self._get_url_cache_name(wwwfile)
        dest_file = self._verified_download_helper(cache_obj_name, _always_none)
        ...
        if not dest_file:
            ...
            dest_file = self.file_downloader(
    ...
    def download_oggm_files(self):
        with self.get_lock():
            return self._download_oggm_files_unlocked()


    def _download_oggm_files_unlocked(self):
        """Checks if the demo data is already on the cache and downloads it."""

        zip_url = 'https://github.com/%s/archive/%s.zip' % \
                  (self.SAMPLE_DATA_GH_REPO, self.SAMPLE_DATA_COMMIT)
        odir = os.path.join(self.oggm.CACHE_DIR)
        sdir = os.path.join(self.oggm.CACHE_DIR,
                            'oggm-sample-data-%s' % self.SAMPLE_DATA_COMMIT)

        # download only if necessary
        if not os.path.exists(sdir):
            ofile = self.file_downloader(zip_url)
    ...
    def _download_srtm_file(self, zone):
        with self.get_lock():
            return self._download_srtm_file_unlocked(zone)


    def _download_srtm_file_unlocked(self, zone):
        ...
        wwwfile = ('http://srtm.csi.cgiar.org/wp-content/uploads/files/srtm_5x5/'
                   'TIFF/srtm_' + zone + '.zip')
        dest_file = self.file_downloader(wwwfile)
        ...
        self.oggm.get_lru_handler(tmpdir).append(outpath)
        return outpath


    def _download_nasadem_file(self, zone):
        with self.get_lock():
            return self._download_nasadem_file_unlocked(zone)


    def _download_nasadem_file_unlocked(self, zone):
    ...
    def _download_tandem_file(self, zone):
        with self.get_lock():
            return self._download_tandem_file_unlocked(zone)


    def _download_tandem_file_unlocked(self, zone):
        ...
        dest_file = self.download_with_authentication(wwwfile, 'geoservice.dlr.de')
    ...
    def _download_dem3_viewpano(self, zone):
        with self.get_lock():
            return self._download_dem3_viewpano_unlocked(zone)


    def _download_dem3_viewpano_unlocked(self, zone):
    ...
    def _download_aster_file(self, zone):
        with self.get_lock():
            return self._download_aster_file_unlocked(zone)


    def _download_aster_file_unlocked(self, zone):
        ...
        dest_file = self.download_with_authentication(wwwfile, 'urs.earthdata.nasa.gov')
    ...
    def _download_topo_file_from_cluster(self, fname):
        with self.get_lock():
            return self._download_topo_file_from_cluster_unlocked(fname)


    def _download_topo_file_from_cluster_unlocked(self, fname):
        ...
        url += fname + '.zip'
        dfile = self.file_downloader(url)
    ...
    def _download_copdem_file(self, cppfile, tilename):
        with self.get_lock():
            return self._download_copdem_file_unlocked(cppfile, tilename)


    def _download_copdem_file_unlocked(self, cppfile, tilename):
        ...
        dest_file = self.download_with_authentication(ftpfile,
        ...
        self.oggm.get_lru_handler(tmpdir).append(demfile)

        return demfile


    def _download_aw3d30_file(self, zone):
        with self.get_lock():
            return self._download_aw3d30_file_unlocked(zone)


    def _download_aw3d30_file_unlocked(self, fullzone):
        ...
        ftpfile = ('ftp://ftp.eorc.jaxa.jp/pub/ALOS/ext1/AW3D30/release_v1804/'
                   + fullzone + '.tar.gz')
        try:
            dest_file = self.file_downloader(ftpfile, timeout=180)
        ...
        for file in os.listdir(dempath):
            self.oggm.get_lru_handler(tmpdir).append(os.path.join(dempath, file))
        return demfile


    def _download_mapzen_file(self, zone):
        with self.get_lock():
            return self._download_mapzen_file_unlocked(zone)


    def _download_mapzen_file_unlocked(self, zone):
        ...
        # That's all
        return self.file_downloader(url, timeout=180)


    def get_prepro_gdir(self, rgi_version, rgi_id, border, prepro_level, base_url=None):
        with self.get_lock():
            return self._get_prepro_gdir_unlocked(rgi_version, rgi_id, border,
                                                  prepro_level, base_url=base_url)


    def _get_prepro_gdir_unlocked(self, rgi_version, rgi_id, border, prepro_level,
                                  base_url=None):
        # Prepro URL
        if base_url is None:
            base_url = self.GDIR_URL
        ...
        tar_base = self.file_downloader(url)
        if tar_base is None:
    ...
    def srtm_zone(self, lon_ex, lat_ex):
    ...
    def _tandem_path(self, lon_tile, lat_tile):
    ...
    def tandem_zone(self, lon_ex, lat_ex):
        ...
        for lat in lat_tiles:
            ...
            for lon in lon_tiles:
                zones.append(self._tandem_path(lon, lat))
        return list(sorted(set(zones)))


    def _aw3d30_path(self, lon_tile, lat_tile):
    ...
    def aw3d30_zone(self, lon_ex, lat_ex):
        ...
        for lon in lon_tiles:
            for lat in lat_tiles:
                zones.append(self._aw3d30_path(lon, lat))
        return list(sorted(set(zones)))


    def _extent_to_polygon(self, lon_ex, lat_ex, to_crs=None):
    ...
    def arcticdem_zone(self, lon_ex, lat_ex):
        """Returns a list of Arctic-DEM zones covering the desired extent.
        """

        gdf = gpd.read_file(self.get_demo_file('ArcticDEM_Tile_Index_Rel7_by_tile.shp'))
        p = _extent_to_polygon(lon_ex, lat_ex, to_crs=gdf.crs)
        gdf = gdf.loc[gdf.intersects(p)]
        return gdf.tile.values if len(gdf) > 0 else []


    def rema_zone(self, lon_ex, lat_ex):
        """Returns a list of REMA-DEM zones covering the desired extent.
        """

        gdf = gpd.read_file(self.get_demo_file('REMA_Tile_Index_Rel1.1.shp'))
        p = _extent_to_polygon(lon_ex, lat_ex, to_crs=gdf.crs)
        gdf = gdf.loc[gdf.intersects(p)]
        return gdf.tile.values if len(gdf) > 0 else []


    def alaska_dem_zone(self, lon_ex, lat_ex):
        """Returns a list of Alaska-DEM zones covering the desired extent.
        """

        gdf = gpd.read_file(self.get_demo_file('Alaska_albers_V3_tiles.shp'))
        p = _extent_to_polygon(lon_ex, lat_ex, to_crs=gdf.crs)
        gdf = gdf.loc[gdf.intersects(p)]
        return gdf.tile.values if len(gdf) > 0 else []


    def copdem_zone(self, lon_ex, lat_ex):
        """Returns a list of Copernicus DEM tarfile and tilename tuples
        """

        # path to the lookup shapefiles
        gdf = gpd.read_file(self.get_demo_file('RGI60_COPDEM_lookup.shp'))
    ...
    def aster_zone(self, lon_ex, lat_ex):
    ...
    def nasadem_zone(self, lon_ex, lat_ex):
    ...
    def mapzen_zone(self, lon_ex, lat_ex, dx_meter=None, zoom=None):
    ...
    def get_rgi_dir(self, version=None, reset=False):
        ...
        ----------
        version : str
            '5', '6', defaults to None (linking to the one specified in oggm.PARAMS)
        reset : bool
        ...
        with self.get_lock():
            return self._get_rgi_dir_unlocked(version=version, reset=reset)


    def _get_rgi_dir_unlocked(self, version=None, reset=False):

        rgi_dir = self.oggm.PATHS['rgi_dir']
        if version is None:
        ...
        if len(glob.glob(test_file)) == 0:
            ...
            ofile = self.file_downloader(dfile, reset=reset)
            # Extract root
            with zipfile.ZipFile(ofile) as zf:
                zf.extractall(rgi_dir)
            # Extract subdirs
            pattern = '*_rgi{}_*.zip'.format(version)
            for root, dirs, files in os.walk(self.oggm.PATHS['rgi_dir']):
    ...
    def get_rgi_region_file(self, region, version=None, reset=False):
        ...
        rgi_dir = self.get_rgi_dir(version=version, reset=reset)
        f = list(glob.glob(rgi_dir + "/*/{}_*.shp".format(region)))
        assert len(f) == 1
        return f[0]


    def get_rgi_glacier_entities(self, rgi_ids, version=None):
        ...
        for reg in sorted(np.unique(regions)):
            sh = gpd.read_file(self.get_rgi_region_file(reg, version=version))
    ...
    def get_rgi_intersects_dir(self, version=None, reset=False):
        ...
        with self.get_lock():
            return self._get_rgi_intersects_dir_unlocked(version=version, reset=reset)


    def _get_rgi_intersects_dir_unlocked(self, version=None, reset=False):
    ...
    def get_rgi_intersects_region_file(self, region=None, version=None, reset=False):
        ...
        version : str
            '5', '6', '61'... defaults the one specified in oggm.PARAMS
        reset : bool
        ...
        rgi_dir = self.get_rgi_intersects_dir(version=version, reset=reset)
    ...
    def get_rgi_intersects_entities(self, rgi_ids, version=None):
        ...
        selection = []
        for reg in sorted(np.unique(regions)):
            sh = gpd.read_file(self.get_rgi_intersects_region_file(reg,
                                                                   version=version))
    ...
    def is_dem_source_available(self, source, lon_ex, lat_ex):
        ...
        def _in_grid(grid_json, lon, lat):
            i, j = self.oggm.DATA['dem_grids'][grid_json].transform(lon, lat,
                                                                    maskout=True)
    ...
    def default_dem_source(self, lon_ex, lat_ex, rgi_region=None, rgi_subregion=None):
    ...
    def get_topo_file(self, lon_ex, lat_ex, rgi_region=None, rgi_subregion=None,
        ...
        if source is not None and not isinstance(source, str):
            ...
            for s in source:
                demf, source_str = self. get_topo_file(lon_ex, lat_ex,
        ...
        if 'dem_file' in self.oggm.PATHS and os.path.isfile(self.oggm.PATHS['dem_file']):
            source = 'USER' if source is None else source
            if source == 'USER':
                return [self.oggm.PATHS['dem_file']], source

        # Some logic to decide which source to take if unspecified
        if source is None:
            source = self.default_dem_source(lon_ex, lat_ex, rgi_region=rgi_region,
                                             rgi_subregion=rgi_subregion)

        if source not in self.DEM_SOURCES:
            raise InvalidParamsError('`source` must be one of '
                                     '{}'.format(self.DEM_SOURCES))

        # OK go
        files = []
        if source == 'GIMP':
            _file = self._download_topo_file_from_cluster('gimpdem_90m_v01.1.tif')
            files.append(_file)

        if source == 'ARCTICDEM':
            zones = self.arcticdem_zone(lon_ex, lat_ex)
            for z in zones:
                with self.get_lock():
                    url = 'https://cluster.klima.uni-bremen.de/~oggm/'
                    url += 'dem/ArcticDEM_100m_v3.0/'
                    url += '{}_100m_v3.0/{}_100m_v3.0_reg_dem.tif'.format(z, z)
                    files.append(self.file_downloader(url))

        if source == 'RAMP':
            _file = self._download_topo_file_from_cluster('AntarcticDEM_wgs84.tif')
            files.append(_file)

        if source == 'ALASKA':
            zones = self.alaska_dem_zone(lon_ex, lat_ex)
            for z in zones:
                with self.get_lock():
                    url = 'https://cluster.klima.uni-bremen.de/~oggm/'
        ...
        if source == 'REMA':
            zones = self.rema_zone(lon_ex, lat_ex)
            for z in zones:
                with self.get_lock():
                    url = 'https://cluster.klima.uni-bremen.de/~oggm/'
        ...
        if source == 'TANDEM':
            zones = self.tandem_zone(lon_ex, lat_ex)
            for z in zones:
                files.append(self._download_tandem_file(z))

        if source == 'AW3D30':
            zones = self.aw3d30_zone(lon_ex, lat_ex)
            for z in zones:
                files.append(self._download_aw3d30_file(z))

        if source == 'MAPZEN':
            zones = self.mapzen_zone(lon_ex, lat_ex, dx_meter=dx_meter, zoom=zoom)
            for z in zones:
                files.append(self._download_mapzen_file(z))

        if source == 'ASTER':
            zones = self.aster_zone(lon_ex, lat_ex)
            for z in zones:
                files.append(self._download_aster_file(z))

        if source == 'DEM3':
            zones = self.dem3_viewpano_zone(lon_ex, lat_ex)
            for z in zones:
                files.append(self._download_dem3_viewpano(z))

        if source == 'SRTM':
            zones = self.srtm_zone(lon_ex, lat_ex)
            for z in zones:
                files.append(self._download_srtm_file(z))

        if source == 'COPDEM':
            filetuple = self.copdem_zone(lon_ex, lat_ex)
            for cpp, eop in filetuple:
                files.append(self._download_copdem_file(cpp, eop))

        if source == 'NASADEM':
            zones = self.nasadem_zone(lon_ex, lat_ex)
            for z in zones:
                files.append(self._download_nasadem_file(z))
    ...
    def get_cmip5_file(self, filename, reset=False):
        ...
        return self.file_downloader(dfile, reset=reset)


    def get_ref_mb_glaciers_candidates(self, rgi_version=None):
        ...
        if rgi_version is None:
            rgi_version = self.oggm.PARAMS['rgi_version']
        ...
        if key not in self.oggm.DATA:
            flink, _ = self.get_wgms_files()
            self.oggm.DATA[key] = flink['RGI{}0_ID'.format(rgi_version)].tolist()

        return self.oggm.DATA[key]


    def get_ref_mb_glaciers(self, gdirs):
        ...
        # Get the links
        ref_ids = self.get_ref_mb_glaciers_candidates(gdirs[0].rgi_version)
